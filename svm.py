...
def normalize(data):
    meanstd = zip( [mu for mu in [np.mean(data[:,j]) for j in range(len(data[0]))]] , [span for span in  [max(data[:,j]) - min(data[:,j]) for j in range(len(data[0]))] ])
    normalizedlst = [ 1.0*(data[i,j]-meanstd[j][0])/meanstd[j][1] for i in range(data.shape[0]) for j in range(len(meanstd))]
    return np.array(normalizedlst,float).reshape(data.shape)

ntraindata = normalize(train_data[0:,1:])
train_data = np.concatenate((train_data[0:,0].reshape((ntraindata.shape[0],1)),ntraindata),axis=1)
...
for times in range(TIMES):
    for paramC in range(1000)[100::10]:
        forest = svm.SVC(C=paramC, kernel='poly',degree=3, gamma=0.15)
#
        ...
        print 'ParamC is ', paramC, ' Error  is ', error
        errorlist.append(error)
        print '---------------------------------------'

error_arr = np.array(errorlist,int)
error_arr = error_arr.reshape((TIMES,len(range(1000)[100::10])))
...
def keyfunc(item):
    return item[1]
print 'Errors for different C param vals'
print 'Error mean ', error_final_list
print 'Std devs for different C param vals'
print std_final_list
decision_lst= zip(range(1000)[100::10],error_final_list)
decision_lst = sorted(decision_lst,key=keyfunc)
print '(C param,Minimum error) is ', decision_lst[0]
print [x for x in decision_lst if x[1]==decision_lst[0][1] ]

#now the test Data

forest = svm.SVC(C=435, kernel='poly',degree=3, gamma=0.15)
forest = forest.fit( train_data[0::,1::], train_data[0::,0] )

print 'Predicting...'
output = forest.predict(normalize(test_data)).astype(int)
